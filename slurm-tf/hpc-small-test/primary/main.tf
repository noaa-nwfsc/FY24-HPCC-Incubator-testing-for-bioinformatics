...
module "mount_data_bucket" {
  source        = "./modules/embedded/modules/file-system/pre-existing-network-storage"
  fs_type       = "gcsfuse"
  local_mount   = "/data"
  mount_options = "defaults,_netdev,implicit_dirs"
  remote_mount  = module.slurm_compute_bucket.gcs_bucket_name
}
...
module "compute_partition" {
  ...
  exclusive            = false
  network_storage      = flatten([module.mount_data_bucket.network_storage, flatten([module.slurm_compute_bucket.network_storage, flatten([module.homefs.network_storage])])])
